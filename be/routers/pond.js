...
router.post('/', async (req, res) => {
    try {
        ...
        const sql = "INSERT INTO POND (ID, name, area, deep, startDate) VALUES (?,?,?,?,?);";

        if (name && area && deep) {

            connection.query(sql, [id, name, area, deep, date], (err, results) => {
                if (err) res.status(500).json(err);
                res.status(200).json(results);
            })
        }
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        sql += "DELETE FROM CROP WHERE CROP.pondID = ?;" +
